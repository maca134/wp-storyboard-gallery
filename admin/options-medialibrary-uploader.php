...
if ( is_admin() ) {
	...
	$wpsbg_of_page= 'appearance_page_options-framework';
	add_action( "admin_print_styles-$wpsbg_of_page", 'wpsbg_optionsframework_mlu_css', 0 );
	add_action( "admin_print_scripts-$wpsbg_of_page", 'wpsbg_optionsframework_mlu_js', 0 );	
}
...
if ( ! function_exists( 'wpsbg_optionsframework_mlu_init' ) ) {
	function wpsbg_optionsframework_mlu_init () {
		register_post_type( 'wpsbg_optionsframework', array(
			'labels' => array(
...
if ( ! function_exists( 'wpsbg_optionsframework_mlu_css' ) ) {

	function wpsbg_optionsframework_mlu_css () {
...
if ( ! function_exists( 'wpsbg_optionsframework_mlu_js' ) ) {

	function wpsbg_optionsframework_mlu_js () {
	
		// Registers custom scripts for the Media Library AJAX uploader.
		wp_register_script( 'of-medialibrary-uploader', WPSBG_OPTIONS_FRAMEWORK_URL .'js/of-medialibrary-uploader.js', array( 'jquery', 'thickbox' ) );
 ...
 *
 * Dependencies:
 * - wpsbg_optionsframework_mlu_get_silentpost()
 */

if ( ! function_exists( 'wpsbg_optionsframework_medialibrary_uploader' ) ) {

	function wpsbg_optionsframework_medialibrary_uploader( $_id, $_value, $_mode = 'full', $_desc = '', $_postid = 0, $_name = '') {
		...
		$int = wpsbg_optionsframework_mlu_get_silentpost( $id );
		...
		if ( $_desc != '' ) {
			$output .= '<span class="wpsbg_of_metabox_desc">' . $_desc . '</span>' . "\n";
		}
		...
		if ( $value != '' ) { 
			...
			if ( $image ) {
				...
				$title = __( 'View File', 'wpsbg_optionsframework' );
 ...
 *
 * Example Usage:
 * wpsbg_optionsframework_mlu_get_silentpost ( 'wpsbg_of_logo' );
 */

if ( ! function_exists( 'wpsbg_optionsframework_mlu_get_silentpost' ) ) {

	function wpsbg_optionsframework_mlu_get_silentpost ( $_token ) {
	
		global $wpdb;
		$_id = 0;
	
		// Check if the token is valid against a whitelist.
		// $_whitelist = array( 'wpsbg_of_logo', 'wpsbg_of_custom_favicon', 'wpsbg_of_ad_top_image' );
		// Sanitise the token.
		...
		if ( $_token ) {
			...
			$_args = array( 'post_type' => 'wpsbg_optionsframework', 'post_name' => 'of-' . $_token, 'post_status' => 'draft', 'comment_status' => 'closed', 'ping_status' => 'closed' );
...
if ( ! function_exists( 'wpsbg_optionsframework_mlu_insidepopup' ) ) {

	function wpsbg_optionsframework_mlu_insidepopup () {
	
		if ( isset( $_REQUEST['is_optionsframework'] ) && $_REQUEST['is_optionsframework'] == 'yes' ) {
		
			add_action( 'admin_head', 'wpsbg_optionsframework_mlu_js_popup' );
			add_filter( 'media_upload_tabs', 'wpsbg_optionsframework_mlu_modify_tabs' );
		}
	}
}

if ( ! function_exists( 'wpsbg_optionsframework_mlu_js_popup' ) ) {

	function wpsbg_optionsframework_mlu_js_popup () {

		$_wpsbg_of_title = $_REQUEST['wpsbg_of_title'];
		if ( ! $_wpsbg_of_title ) { $_wpsbg_of_title = 'file'; } // End IF Statement
?>
	...
	jQuery(function($) {
		...
		$( 'h3.media-title' ).each ( function () {
			...
			var new_title = current_title.replace( 'media file', '<?php echo $_wpsbg_of_title; ?>' );
			$( this ).html( new_title );
...
if ( ! function_exists( 'wpsbg_optionsframework_mlu_modify_tabs' ) ) {

	function wpsbg_optionsframework_mlu_modify_tabs ( $tabs ) {
		$tabs['gallery'] = str_replace( __( 'Gallery', 'wpsbg_optionsframework' ), __( 'Previously Uploaded', 'wpsbg_optionsframework' ), $tabs['gallery'] );
		return $tabs;
	}
}